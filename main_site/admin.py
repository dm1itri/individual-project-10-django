...
class QuestionAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ('Ответы', {'fields': ['answer_correct', 'answer_2', 'answer_3', 'answer_4'], 'classes': ['collapse']}),
    ]


class GameAdmin(admin.ModelAdmin):
    list_display = ['current_player', 'number_of_players', 'number_of_players_connected',  'is_started', 'is_over', 'datetime_creation']
    list_filter = ['is_started', 'is_over', 'datetime_creation']


class HistoryMoveAdmin(admin.ModelAdmin):
    list_display = ['game_id', 'number_history', 'number_move', 'number_steps', 'datetime_addition']
